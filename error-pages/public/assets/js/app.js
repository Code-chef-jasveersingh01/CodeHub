    ...
    function pluginData() {
        ...
        Array.from(flatpickrExamples).forEach(function (item) {
            if (item.getAttribute("data-provider") == "flatpickr") {
                ...
                if (isFlatpickerVal["data-lang"]) {
                    dateData.locale = isFlatpickerVal["data-lang"].value.toString();
                }
                // for hijri calander
                if (isFlatpickerVal["data-calender-type"]) {
                    if (isFlatpickerVal["data-calender-type"].value.toString() == 2) {
                        dateData.plugins = [
...
function mapObject(obj, key) {
    return obj && typeof obj === "object" &&
        (Object.keys(obj).includes(key) ||
            Object.values(obj).some(o => mapObject(o, key)));
}

function mapObjectLangValue(obj, key) {
    if (key === 'en') {
        return mapObject(obj, key) ? obj.name.en : '';
    }
    if (key === 'ar') {
        return mapObject(obj, key) ? obj.name.ar : (mapObject(obj, 'en') ? obj.name.en : '');
    }
    if (key === 'de') {
        return mapObject(obj, key) ? obj.name.de : (mapObject(obj, 'en') ? obj.name.en : '');
    }
}

$(function () {
    $(".mutli-lang").click(function (e) {
        ...
        if (lang_type == 'en') {
            ...
            $("" + field_type + "[name='" + lang_field + "[de]']").hide();
            $(this).parent().children().css("color", "");
            $(this).css("color", "#1dbfc1");
            ...
            $(this).parent().children().css("color", "");
            $(this).css("color", "#1dbfc1");
        } else if (lang_type == 'de') {
            $("" + field_type + "[name='" + lang_field + "[en]']").hide();
            $("" + field_type + "[name='" + lang_field + "[ar]']").hide();
            $("" + field_type + "[name='" + lang_field + "[de]']").show();
            $(this).parent().children().css("color", "");
            $(this).css("color", "#1dbfc1");
        }
    });
...
function searchDataTableFilter(table) {
    $('#' + table + '').DataTable().ajax.reload();
}

function searchOnTypeDataTableFilter(table) {
    if ($("#filter_search_key").val().replace(/\s/g, '').length >= 2) {
        $('#' + table + '').DataTable().ajax.reload();
    }
}

function resetDataTableFilter(table) {
    if ($("#filter_search_key").length) $('#filter_search_key').val('');
    if ($(".department_record").length) $('.department_record:checked').map(function () { $(this).prop('checked', false); $('#departmentCustomDropDown').val(''); });
    if ($("#filter_manager").length) $('#filter_manager').val("").trigger("change");
    if ($("#filter_shift").length) $('#filter_shift').val("").trigger('change');
    if ($("#filter_role_ids").length) $('#filter_role_ids').val("").trigger("change");
    if ($("#filter_gender_type").length) $('#filter_gender_type').val("").trigger("change");
    if ($("#filter_nationality_type").length) $('#filter_nationality_type').val("").trigger("change");
    if ($("#filter_location_type").length) $('#filter_location_type').val("").trigger("change");
    if ($("#filter_contract_type").length) $('#filter_contract_type').val("").trigger("change");
    if ($("#filter_job_title_type").length) $('#filter_job_title_type').val("").trigger("change");
    if ($("#filter_status").length) $('#filter_status').val("all").trigger("change");
    if ($("#filter_upload_type").length) $('#filter_upload_type').val("").trigger("change");
    if ($("#filter_leave_type").length) $('#filter_leave_type').val("").trigger("change");
    if ($("#filter_short_leave_type").length) $('#filter_short_leave_type').val("").trigger("change");
    if ($("#filter_request_date").length) $('#filter_request_date').val("");
    if ($("#filter_upload_date").length) $('#filter_upload_date').val("");
    if ($("#filter_shift_type").length) $('#filter_shift_type').val("").trigger("change");
    if ($("#filter_attendance_type").length) $('#filter_attendance_type').val("").trigger("change");
    if ($("#filter_date").length) $('#filter_date').val("");
    if ($("#filter_priority").length) $('#filter_priority').val("").trigger("change");
    if ($("#filter_type").length) $('#filter_type').val("").trigger("change");
    if ($("#filter_created_at").length) $('#filter_created_at').val("");
    if ($("#filter_login_at").length) $('#filter_login_at').val("");
    if ($("#filter_apply_date").length) $('#filter_apply_date').val("");
    if ($("#filter_effective_date").length) $('#filter_effective_date').val("");
    if ($("#filter_event").length) $('#filter_event').val("").trigger("change");
...
function showTimer(seconds) {
    ...
    setInterval(function () {
        var a = new Date();
        document.getElementById("show-timer-section").innerHTML = sec;
        sec--;
        if (sec == '00') {
            sec = seconds;
        }
